...
public class Seam {
	...
	* Given an image find the vertical seam that should be removed 
	* store the seam in an array.
	* @param im CSBSJUImage
	* @return int[] that stores the pixels to remove
	*/
			...
			for (int x = 0; x < im.getWidth(); x++) {
				...
				for (int k = x - 1; k <= x + 1; k++) {
					...
					if (k >= 0 && k < im.getWidth()) {
						...
						if (dist < distTo[x][y]) {
							// found a better path
							distTo[x][y] = dist; // record new distance
	...
	public double energy(int x, int y, CSBSJUImage im) {
		if (x < 0 || y < 0 || x >= im.getWidth() || y >= im.getHeight())
			...
			//implement this based on formula
	...
	/**
	* Heather is giving you this helper
	...
	*/
	private Color colorOfPixel(int x, int y, CSBSJUImage im) {
		int W = im.getWidth();
		int H = im.getHeight();
		return new Color(im.getRed((x + W) % W, (y + H) % H), im.getGreen((x + W) % W, (y + H) % H),
				im.getBlue((x + W) % W, (y + H) % H));
	}
	...
	*/
	private double calculateGradient(Color p1, Color p2) {
		//implement this
		double r = abs(p1.getRed() - p2.getRed());
		double g = abs(p1.getGreen() - p2.getGreen());
		double b = abs(p1.getBlue() - p2.getBlue());
		return r * r + g * g + b * b;
	}
